    ...
    var table = $('#notificheTable').DataTable({
        "ajax": {
            ...
            "dataSrc": function (json) {
                $('#notificaBadge .text-danger').text(json.totalNotifiche || 0);
                return json.aaData || [];
            },
            "dataType": "json"
        },
...
function mostraNotifiche() {
    ...
    $.ajax({
        ...
        success: function (data) {
            $('#notificaBadge .text-danger').text(data.totalNotifiche);
        },
        error: function () {
...
function segnaComeLetto() {
    $.ajax({
        ...
        data: {read: true},
        success: function () {
            $('#notificaBadge .text-danger').text('0');
            console.log("Notifiche segnate come lette.");
            location.reload();  // Ricarica la pagina per aggiornare le notifiche
        },
        error: function () {
...
document.getElementById("richiediPermessoForm").addEventListener("submit", function (event) {
    ...
    // Configura i parametri di isCheck per il controllo iniziale
...
function invioRichiestaSenzaOre() {
    ...
    fetch("RichiestaPermessoServlet", {
        ...
        body: formData
            ...
            .then(response => {
                if (response.ok) {
                    window.location.href = "US_gestionale.jsp?esito=OK&codice=001";
                } else {
