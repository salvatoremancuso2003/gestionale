    ...
    * @
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            StringBuilder queryString = new StringBuilder("SELECT u FROM Utente u WHERE u.ruolo.id = 2 AND u.status != 0");
            ...
            for (Utente utente : utenti) {
                ...
                String editUser = "<div class='col-auto'>"
                        + " <button class='btn btn-danger' style='min-width: 100px; margin-left: 5px;' "
                        + " data-fancybox "
                        + " href='edit_user.jsp?userId=" + utente.getId() + "'"
                        + " data-type='iframe' "
                        + " data-options='{\"iframe\" : {\"css\": {\"width\": \"90%\", \"height\": \"90%\"}}}'>"
                        + "Modifica utente</button>"
                        + "</div>";

                rc.addProperty("Gestisci", editUser);
                dataArray.add(rc);
            }
